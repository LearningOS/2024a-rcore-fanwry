...
impl TaskManager {
    ...
    //fan:calculate the time
    fn task_run_time(&self) -> usize {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        get_time_ms() - inner.tasks[current].task_start_time
    }
...
///fan
pub fn task_run_time() -> usize {
    TASK_MANAGER.task_run_time()
}
